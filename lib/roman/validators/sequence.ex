defmodule Roman.Validators.Sequence do
  ...
  defp check_increasing_value_order([{_, %{value: a}}, {_, %{value: b}} | _] = [_ | t])
       when a >= b do
    check_increasing_value_order(t)
  end

  defp check_increasing_value_order([{num_l, %{value: val_l}}, {num_r, %{value: val_r}} | _]) do
    {:error,
     {:sequence_increasing,
  ...
  # MCMD or CMC.
  @spec subtraction_bounds_following_values(sequence) :: {:ok, sequence} | Roman.error()
  ...
  @spec check_subtraction_bound(
          sequence,
          nil | {integer, Roman.numeral()}
        ) :: :ok | Roman.error()
  ...
  defp check_subtraction_bound([{num, %{value: v}} | _], {delta, delta_num})
       when v >= delta do
    {:error,
     {:value_greater_than_subtraction,
      "once a value has been subtracted from another, no further numeral or " <>
        ...
        "#{num} (#{v}) after having previously subtracted #{delta} " <> "(in #{delta_num})"}}
  end
end
